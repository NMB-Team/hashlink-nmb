...
HL_PRIM bool HL_NAME(event_loop)( event_data *event ) {
	while (true) {
		...
		case SDL_EVENT_FINGER_DOWN:
			...
			event->mouseX = e.tfinger.x * 10000;
			event->mouseY = e.tfinger.y * 10000;
...
HL_PRIM int HL_NAME(warp_mouse_global)(float x, float y) {
	return SDL_WarpMouseGlobal(x, y);
}

HL_PRIM void HL_NAME(warp_mouse_in_window)(SDL_Window* window, float x, float y) {
...
HL_PRIM bool HL_NAME(win_set_display_mode)(SDL_Window *win, int width, int height, float framerate) {
