...
static SDL_DisplayID display_id_from_index( int index ) {
	int count;
...
static int display_index_from_id( SDL_DisplayID display ) {
	int count;
	SDL_DisplayID primary = SDL_GetPrimaryDisplay();
	SDL_DisplayID *displays = SDL_GetDisplays(&count);
	if( display == primary ) {
...
HL_PRIM int HL_NAME(get_screen_width_of_window)(SDL_Window* win) {
	const SDL_DisplayMode *e = SDL_GetCurrentDisplayMode(win != NULL ? SDL_GetDisplayForWindow(win) : display_id_from_index(0));
	return e->w;
}
...
HL_PRIM int HL_NAME(win_display_handle)(SDL_Window *win) {
	return display_index_from_id(SDL_GetDisplayForWindow(win));
}
...
HL_PRIM varray* HL_NAME(get_displays)() {
	...
	for (int i = 0; i < n; i++) {
		...
		SDL_Rect rect;
		SDL_DisplayID display = display_id_from_index(i);
...
HL_PRIM varray* HL_NAME(get_display_modes)(int display_id) {
	...
	int count;
	modes = SDL_GetFullscreenDisplayModes(display_id_from_index(display_id), &count);
	if( modes == NULL )
		return NULL;
...
HL_PRIM vdynobj* HL_NAME(get_current_display_mode)(int display_id, bool registry) {
	...
	SDL_DisplayID display = display_id_from_index(display_id);
	if(registry)
