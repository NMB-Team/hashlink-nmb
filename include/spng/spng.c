...
static struct spng__iter spng__iter_init(unsigned bit_depth, const unsigned char* samples) {
	struct spng__iter iter = { .mask = (uint32_t)(1 << bit_depth) - 1, .shift_amount = 8 - bit_depth, .initial_shift = 8 - bit_depth, .bit_depth = bit_depth, .samples = samples };
...
*/
static int defilter_scanline(const unsigned char* prev_scanline, unsigned char* scanline, size_t scanline_width, unsigned bytes_per_pixel, unsigned filter) {
...
static int filter_scanline(unsigned char* filtered, const unsigned char* prev_scanline, const unsigned char* scanline, size_t scanline_width, unsigned bytes_per_pixel, const unsigned filter) {
...
static int32_t filter_sum(const unsigned char* prev_scanline, const unsigned char* scanline, size_t size, unsigned bytes_per_pixel, const unsigned filter) {
...
static unsigned get_best_filter(const unsigned char* prev_scanline, const unsigned char* scanline, size_t scanline_width, unsigned bytes_per_pixel, const int choices) {
	if (!choices)
...
static inline void trns_row(unsigned char* row, const unsigned char* scanline, const unsigned char* trns, unsigned scanline_stride, struct spng_ihdr* ihdr, uint32_t pixels, int fmt) {
	uint32_t i;
	unsigned row_stride;
...
static void expand_row(unsigned char* row, const unsigned char* scanline, const union spng__decode_plte* decode_plte, uint32_t width, int fmt) {
	uint32_t i = 0;
	unsigned char* px;
	unsigned char entry;
...
static int check_chrm_int(const struct spng_chrm_int* chrm_int) {
	...
	if (chrm_int->white_point_x > spng_u32max || chrm_int->white_point_y > spng_u32max || chrm_int->red_x > spng_u32max || chrm_int->red_y > spng_u32max || chrm_int->green_x > spng_u32max || chrm_int->green_y > spng_u32max ||
...
static int is_small_chunk(uint8_t type[4]) {
	if (!memcmp(type, type_plte, 4))
		...
		return 1;
	else if (!memcmp(type, type_actl, 4))
		return 1;
	else if (!memcmp(type, type_fctl, 4))
		return 1;
	else
		return 0;
}
...
static int read_chunks(spng_ctx* ctx, int only_ihdr) {
	...
	if (ctx->state == SPNG_STATE_FRAME_IDLE)
		return 0;
...
int spng_decode_image(spng_ctx* ctx, void* out, size_t len, int fmt, int flags) {
	...
	if (fmt & (SPNG_FMT_RGBA8 | SPNG_FMT_RGBA16)) {
		...
		f.apply_trns = 0;
	} else if (fmt == SPNG_FMT_GA8 && (ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE || ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE_ALPHA) && ihdr->bit_depth <= 8) {
		if (ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE_ALPHA && ihdr->bit_depth == depth_target)
			f.same_layout = 1;
		else if (ihdr->bit_depth <= 8)
			f.unpack = 1;
	} else if (fmt == SPNG_FMT_GA16 && (ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE || ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE_ALPHA) && ihdr->bit_depth == 16) {
		if (ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE_ALPHA && ihdr->bit_depth == depth_target)
			f.same_layout = 1;
	...
	if (!depth_target || depth_target > 16 || !processing_depth || processing_depth > 16 || !sb->grayscale_bits || sb->grayscale_bits > processing_depth || !sb->alpha_bits || sb->alpha_bits > processing_depth || !sb->red_bits ||
		sb->red_bits > processing_depth || !sb->green_bits || sb->green_bits > processing_depth || !sb->blue_bits || sb->blue_bits > processing_depth) {
		return decode_err(ctx, SPNG_ESBIT);
	}

	if (sb->red_bits == sb->green_bits && sb->green_bits == sb->blue_bits && sb->blue_bits == sb->alpha_bits && sb->alpha_bits == processing_depth && processing_depth == depth_target)
		f.do_scaling = 0;
...
static int encode_scanline(spng_ctx* ctx, const void* scanline, size_t len) {
	...
	if (ret == SPNG_EOI) {
		int error;
		if (ctx->reading_fdat)
			error = finish_fdat(ctx);
		else
			error = finish_idat(ctx);
...
spng_ctx* spng_ctx_new2(struct spng_alloc* alloc, int flags) {
	...
	const struct spng__zlib_options text_defaults = {
		.compression_level = Z_DEFAULT_COMPRESSION, .window_bits = 15, .mem_level = 8, .strategy = Z_DEFAULT_STRATEGY, .data_type = 1 /* Z_TEXT */
	};
